...
- **Status: DONE. Zac closed this out fully** — Connect Vars + the 12h schedule are set on `intangibles-pin-range-plays-2026` (GUID `eb324534-ebb9-409f-a027-a716f591688b`), pins live, OF and IF both fast. Nothing outstanding on this thread.
- **EXACT next step:** Nothing here — pick up one of the ALSO OPEN threads below, or new work. If Range & Difficulty ever shows a play that looks wrong, `scripts/diff_range_pin.py` is the tool (`--mode batch` for the IN-list change, `--mode pin` for the parquet round-trip; exit 0 clean / 1 real differences / 3 wrong domain for that player / 4 inconclusive).
